...
Object.defineProperty(Builder.prototype, 'name', { get () { return this.proto.table === undefined ? this.proto.name : this.proto.table } })
Object.defineProperty(Builder.prototype, 'vals', { get () { return this._vals } })
...
Builder.prototype._where  = function(...options) {
  ...
  if (typeof el0 == 'string') {
    ...
    return el2 === null
      ? `${quoteName1(this.name)}.${quoteName1(el0)} ${el1} NULL`
      : `${quoteName1(this.name)}.${quoteName1(el0)} ${el1} ${Array.isArray(el2) ? `(${el2.map(quoteName2).join(',')})` : quoteName2(el2)}`
  }
...
Builder.prototype._updateString = function() {
  ...
  for (let key in this.option.attrs) {
    sets.push(`${quoteName1(this.name)}.${quoteName1(key)}=?`)
    vals.push(this.option.attrs[key])
  }
