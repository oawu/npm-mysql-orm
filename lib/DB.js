...
const logger = (error, startAt, sql, vals) => {
  return DB.config.queryLogDir && FileSystem.writeFile(
    DB.config.queryLogDir + DateTime('date') + '.log',
    `${DateTime()} ➜ ${Date.now() - startAt}ms │ ${error ? 'FAIL' : 'OK'} ➜ ${sql} [${vals}]${error ? ` │ ${error.message || error.stack}` : ''}\n`,
...
const execute = (sql, closure) => {
  ...
  ? connection.query(sqlStr, vals, (error, data) => connection.release(!error
    ? closure(logger(null, startAt, sqlStr, vals.join(', ')), data)
    : closure(logger(error, startAt, sqlStr, vals.join(', ')), null)))
  : closure(logger(error, startAt, sqlStr, vals.join(', ')), null))
}
